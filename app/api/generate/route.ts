...
import { hashBuffer, saveUploadedFile, downloadAndSaveImage } from '@/lib/imageProcessing';
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const sceneBasePath = path.join(process.cwd(), 'public', 'scenes', 'Candle Labels');
    const sceneImagePath = path.join(sceneBasePath, 'CandleLabels_Large.jpg');
    const maskImagePath = path.join(sceneBasePath, 'CandleLabels_Large_Mask.png');
